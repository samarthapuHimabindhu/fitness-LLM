...
//   // Memoize handleTimeUpdate to ensure consistency in useEffect
...
//       if (duration > 0) {
//         updateProgress(videoSrc, currentTime, duration); // Update progress
//       }
//     }
...
//         // Force a time update after 1 second of play
//         setTimeout(() => {
//           handleTimeUpdate();
//         }, 1000);
//       });
...
const VideoPlayer = ({ videoSrc, goBackToSelection, updateProgress }) => {
  ...
  const handlePoseDetected = (landmarks) => {
    ...
    if (Math.abs(leftWrist.y - rightWrist.y) < 0.05) {
      ...
      if (!isPlaying) {
        setFeedback('Good posture! Keep going!');
        setIsPlaying(true); // Set video as playing
        videoRef.current?.play();
      }
    } else {
      // Incorrect posture detected, pause the video
      if (isPlaying) {
        setFeedback('Incorrect posture detected! Pausing...');
